<?php
/**
 * Created by PhpStorm.
 * User: MPROTO
 * Date: 15/03/2019
 * Time: 09:27 AM
 */
?>

<?php
?>

        <!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="utf-8">
    <style>
        body {
            font-family: "Trebuchet MS";
        }

        .col-tit{
            margin: 2px;
            background-color: lightgrey;
            text-align: center;
            font-weight: bold;
            font-size: 9px;
            padding: 5px;
        }

        .col-cont{
            margin: 2px;
            text-align: center;
            font-size: 9px;
            padding: 5px;
            border-bottom: 1px solid lightgrey;
        }

        .tit {
            font-weight: bold;
            font-size: 9px;
            letter-spacing: 2px;
        }
        .subtit {
            font-size: 9px;
            letter-spacing: 2px;
        }


        .tabledetll tr td {
            border-bottom: 1px solid lightgrey;
            text-align: center;
            font-size: 7px;
        }

        .tablegral, .tablegral th, .tablegral td {
            border: 1px solid black;
            text-align: center;
        }

        .tablegral thead .gral th {
            width: 150px;
            background-color: lightgrey;
            text-align: center;

        }
        .tabledetll tbody tr td {
            font-size: 9px;
            padding: 3px;
        }

        .tabledetll thead tr th {
            background-color: lightgrey;
            font-size: 9px;
            font-weight: bold;
            padding: 5px;
            text-align: center;
        }

        .footer {
            text-align: center;
            font-size: 8px;
            letter-spacing: 2px;
            font-weight: bold;
        }
        #gral{
            margin-top: 50px;
        }

        #foto {
            position: absolute;
            background-color: lightgrey;
            top: -20px;
            left: 670px;
            right: 0;
            margin: 0 auto;
            width: 2cm;
            height: 2.2cm;
            opacity: 0.5;

        }
        #foto p{
            text-align:center;
            margin-top: 35px ;
            font-weight: bold;
        }


    </style>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">

</head>
<body>

<div class="section ">
    <div style="padding: 0.5cm" class="page">
        <div class="dtll">
            <div >
                <img
                        src="data:image/png;base64,"
                        width="20%" >
                <div style="float: right; text-align: center; width: 400px; background-color: lightgrey; padding: 9px; font-size: 13px ">
                    https://paenms.seq.gob.mx o https://siem.seq.gob.mx<br>
                    <b>Fecha de registro: </b>1 de Abril al 15 de Mayo de 2019
                </div>
            </div>


            <div id="gral" style="text-align:center;">
                <h6><b>Pago de Derechos</b></h6>
            </div>

            <div style="text-align:center">
                <div style="text-align: right; padding-right: 13px ">
                    <b>Folio: </b>{{ get_aspirante()->folio }}
                </div>
                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-tit" style=" width: 70%;">NOMBRE(S), APELLIDO PATERNO, APELLIDO MATERNO</div>
                    <div class="col-tit" style=" width: 28%;">FECHA ALTA</div>
                </div>
                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-cont" style=" width: 70%;">{{$datos->contribuyente->nombre_completo}}</div>
                    <div class="col-cont" style=" width: 28%;">{{$datos->fecha_solicitud }}</div>
                </div>

                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-tit" style=" width: 49%;">REGISTRO FEDERAL DE CONTRIBUYENTES</div>
                    <div class="col-tit" style=" width: 49%;">CLAVE ÚNICA DE REGISTRO DE POBLACIÓN</div>
                </div>
                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-cont" style=" width: 49%;">
                        @if($datos->contribuyente->rfc == null)
                            (sin especificar)
                        @else
                            {{$datos->contribuyente->rfc }}
                        @endif
                    </div>
                    <div class="col-cont" style=" width: 49%;">
                        @if($datos->contribuyente->curp == null)
                            (sin especificar)
                        @else
                            {{$datos->contribuyente->curp }}
                        @endif
                    </div>
                </div>

                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-tit" style=" width: 70%;">CALLE O CRUZAMIENTOS</div>
                    <div class="col-tit" style=" width: 28%;">COLONIA</div>
                </div>
                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-cont" style=" width: 70%;">{{strtoupper($datos->contribuyente->calle)}}</div>
                    <div class="col-cont" style=" width: 28%;">{{strtoupper($datos->contribuyente->colonia)}}</div>
                </div>

                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-tit" style=" width: 70%;">NÚMERO</div>
                    <div class="col-tit" style=" width: 28%;">CÓDIGO POSTAL</div>
                </div>
                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-cont" style=" width: 70%;">{{$datos->contribuyente->numero}}</div>
                    <div class="col-cont" style=" width: 28%;">
                        @if($datos->contribuyente->codigo_postal == null)
                            (sin especificar)
                        @else
                            {{$datos->contribuyente->codigo_postal }}
                        @endif
                    </div>
                </div>

                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-tit" style=" width: 32.5%;">LOCALIDAD</div>
                    <div class="col-tit" style=" width: 32.5%;">MUNICIPIO</div>
                    <div class="col-tit" style=" width: 32.5%;">ENTIDAD FEDERATIVA</div>
                </div>
                <div class="row" style="margin: auto; width: 100%;">
                    <div class="col-cont" style=" width: 32.5%;">{{strtoupper($datos->contribuyente->localidad) }}</div>
                    <div class="col-cont" style=" width: 32.5%;">{{strtoupper($datos->contribuyente->municipio) }}</div>
                    <div class="col-cont" style=" width: 32.5%;">{{strtoupper($datos->contribuyente->entidad_federativa) }}</div>
                </div>
            </div>

            <div class="text-justify" style="font-size: 13px;padding: 8px">
                SE PAGARÁN ESTOS DERECHOS CON FUNDAMENTO EN EL ARTÍCULO 207-H DE LA LEY DE HACIENDA DEL ESTADO DE QUINTANA ROO, VIGENTE.
            </div>

            <div class="text-center font-weight-bold" style="padding: 10px">
                CONCEPTO DE PAGO
            </div>

            <table class=" tabledetll">
                <thead>
                <tr>
                    <th style="width: 300px;">DESCRIPCIÓN</th>
                    <th style="width: 150px">CANTIDAD</th>
                    <th style="width: 200px">NUM. SALARIOS MÍNIMOS</th>
                    <th style="width: 125px">SALARIO MÍNIMO</th>
                </tr>
                </thead>
                <tbody>
                    <tr style="background-color: rgba(217,216,216,0.3)">
                        <td >{{$datos->concepto->descripcion }}</td>
                        <td >1</td>
                        <td >2.14</td>
                        <td >102.68</td>
                    </tr>
                </tbody>
            </table>

            <div class="text-right" style="padding: 10px">
                <b>IMPORTE TOTAL A PAGAR: </b> ${{$datos->total }}
            </div>

            <div class="text-justify font-weight-bold" style="font-size: 13px;padding: 10px">
                Por favor acuda a cualquiera de los bancos autorizados para realizar su pago.
            </div>

            <div class="text-justify font-weight-bold" style="font-size: 13px;padding: 10px">
                Si requiere de la expedición de un recibo oficial, deberá acudir con su comprobante de pago a cualquier
                unidad de los Servicios Educativos del Quintana Roo, ubicadas en los municipios de Othón P. Blanco
                (Av. Insurgentes No. 600), Felipe Carrillo Puerto (Av. Santiago Pacheco Cruz esq. Calle 53), Solidaridad
                (Av. Constituyentes entre Av. 95 y 100), Cozumel (5ta Av. entre 2 y 4 Norte) y Benito Juárez (Av.
                Bonampak, entre Av. Chichen Itzá y García de la Torre), en un horario de 11:00 a 15:00 hrs.
            </div>

            <table style="margin: 0 auto;">
                <tr>
                    <td width="300px" style="text-align: center; border: 3px solid rgba(217,216,216,0.7)" >
                        Fecha límite de la referencia:<br>
                        {{ $datos->referencia_pago->vigencia }}
                    </td>
                    <td width=400px" style="background-color: rgba(217,216,216,0.7); border: 3px solid rgba(217,216,216,0.7)">
                        <p style="line-height: 12px; font-size: 13px; padding: 10px; text-align: justify">La boleta de pago es personal y tiene una clave de referencia
                        única por usuario, por lo que no deberá ser fotocopiado por el
                            pago de otro usuario.</p>
                    </td>
                </tr>
            </table>

            <div >
                <p class="font-weight-bold" style="font-size:13px ">Convenio de pago</p>
                <table>
                    <tr>
                        <td width="150px" class="font-weight-bold" style="font-size:13px ">Banco</td>
                        <td width="150px" class="font-weight-bold" style="font-size:13px ">Convenio</td>
                        <td width="250px" class="font-weight-bold" style="font-size:13px ">Preferencia</td>
                    </tr>
                    <tr style="font-size:13px ">
                        <td>SANTANDER</td>
                        <td>3776</td>
                        <td>{{ $datos->referencia_pago->referencia }}</td>
                    </tr>
                    <tr style="font-size:13px ">
                        <td>HSBC</td>
                        <td>1072</td>
                        <td>{{ $datos->referencia_pago->referencia }}</td>
                    </tr>
                </table>
            </div>

            <div class="text-justify">
                <p style="font-size: 9px; line-height:1" class="mt-3">
                    <i><strong>Nota:</strong>
                        Después del pago efectuado, de 1 a 3 días, ingresa al portal de registro, para poder imprimir "Pase al Examen". Recuerda que después de la fecha límite de la referencia bancaria, no podrás realizar el pago.
                    </i>
                </p>
            </div>

        </div>
    </div>
</div>
</body>
</html>
