<?php
/**
 * Created by PhpStorm.
 * User: MPROTO
 * Date: 13/03/2019
 * Time: 11:38 AM
 */
?>

<?php
?>

    <!DOCTYPE html>
<html lang="es">
    <head>
        <meta charset="utf-8">
        <style>
            body {
                font-family: "Trebuchet MS";
            }

            .col-tit {
                margin: 2px;
                background-color: lightgrey;
                text-align: center;
                font-weight: bold;
                font-size: 9px;
                padding: 5px;
            }

            .col-cont {
                margin: 2px;
                text-align: center;
                font-size: 9px;
                padding: 5px;
                border-bottom: 1px solid lightgrey;
            }

            .tit {
                font-weight: bold;
                font-size: 9px;
                letter-spacing: 2px;
            }

            .subtit {
                font-size: 9px;
                letter-spacing: 2px;
            }


            .tabledetll tr td {
                border-bottom: 1px solid lightgrey;
                text-align: center;
                font-size: 7px;
            }

            .tablegral, .tablegral th, .tablegral td {
                border: 1px solid black;
                text-align: center;
            }

            .tablegral thead .gral th {
                width: 150px;
                background-color: lightgrey;
                text-align: center;

            }

            .tabledetll tbody tr td {
                font-size: 9px;
                padding: 3px;
            }

            .tabledetll thead tr th {
                background-color: lightgrey;
                font-size: 9px;
                font-weight: bold;
                padding: 5px;
            }

            .footer {
                text-align: center;
                font-size: 8px;
                letter-spacing: 2px;
                font-weight: bold;
            }

            #gral {
                position: relative;
                margin-top: 26px;
                margin-bottom: 25px;
            }

            #foto {
                position: absolute;
                background-color: lightgrey;
                top: -20px;
                left: 670px;
                right: 0;
                margin: 0 auto;
                width: 2cm;
                height: 2.2cm;
                opacity: 0.5;

            }

            #foto p {
                text-align: center;
                margin-top: 35px;
                font-weight: bold;
            }


        </style>
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">

    </head>
    <body>
        <div class="section ">
            <div style="padding: 0.5cm" class="page">
                <div class="dtll">
                    <div>
                        <img
                            src="data:image/png;base64,"
                            width="20%">
                        <div style="float: right; text-align: center; width: 400px; background-color: lightgrey; padding: 2px; font-size: 13px ">
                            https://paenms.seq.gob.mx o https://siem.seq.gob.mx<br>
                            <b>Fecha de registro: </b>1 de Abril al 15 de Mayo de 2019
                        </div>

                    </div>


                    <div id="gral" style="text-align:center">
                        <h6>
                            @if($aspirante[0]->informacionProcedencia->fecha_egreso == 2019)
                                <b>PASE AL EXAMEN</b><br>PARA ALUMNOS DE TERCERO DE SECUNDARIA
                            @else
                                <b>PASE AL EXAMEN</b><br>PARA ALUMNOS EGRESADOS EN AÑOS ANTERIORES
                            @endif
                        </h6>
                    </div>

                    <div style="text-align:center">

                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-tit" style=" width: 19%;">FECHA DE NACIMIENTO</div>
                            <div class="col-tit" style=" width: 60%;">CLAVE ÚNICA DE REGISTRO DE POBLACIÓN</div>
                            <div class="col-tit" style=" width: 19%;">FOLIO CENEVAL</div>
                        </div>
                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-cont" style=" width: 19%;">{{ \Carbon\Carbon::parse($aspirante[0]->fecha_nacimiento)->format('d/m/Y')}}</div>
                            <div class="col-cont" style=" width: 60%;">{{ $aspirante[0]->curp }}</div>
                            <div class="col-cont" style=" width: 19%;">{{ $aspirante[0]->folio }}</div>
                        </div>

                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-tit" style=" width: 30%;">APELLIDOS</div>
                            <div class="col-tit" style=" width: 50%;">NOMBRE(S)</div>
                            <div class="col-tit" style=" width: 19%;">SEXO</div>
                        </div>
                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-cont" style=" width: 30%;">{{$aspirante[0]->user->primer_apellido.' '.$aspirante[0]->user->segundo_apellido}}</div>
                            <div class="col-cont" style=" width: 50%;">{{$aspirante[0]->user->nombre}}</div>
                            <div class="col-cont" style=" width: 19%;">
                                @if($aspirante[0]->sexo == 'M')
                                    MUJER
                                @else
                                    HOMBRE
                                @endif
                            </div>
                        </div>

                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-tit" style=" width: 32%;">ENTIDAD FEDERATIVA</div>
                            <div class="col-tit" style=" width: 34%;">MUNICIPIO</div>
                            <div class="col-tit" style=" width: 32%;">LOCALIDAD</div>
                        </div>
                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-cont" style=" width: 32%;">{{$aspirante[0]->domicilio->localidad->NOM_ENT}}</div>
                            <div class="col-cont" style=" width: 34%;">{{$aspirante[0]->domicilio->localidad->NOM_MUN}}</div>
                            <div class="col-cont" style=" width: 32%;">{{$aspirante[0]->domicilio->localidad->NOM_LOC}}</div>
                        </div>

                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-tit" style=" width: 36%;">CCT DE LA ESCUELA SECUNDARIA</div>
                            <div class="col-tit" style=" width: 63%;">NOMBRE DE LA ESCUELA SECUNDARIA</div>
                        </div>
                        <div class="row" style="margin: auto; width: 100%;">
                            <div class="col-cont" style=" width: 36%;">{{$aspirante[0]->informacionProcedencia->clave_centro_trabajo}}</div>
                            <div class="col-cont" style=" width: 63%;">{{ strtoupper($aspirante[0]->informacionProcedencia->nombre_centro_trabajo)}}</div>
                        </div>

                        <div class="text-center font-weight-bold" style="font-size: 13px;padding: 10px">
                            Orden de preferencia en las opciones educativas seleccionadas (plantel y especialidad)
                        </div>

                        <table class=" tabledetll">
                            <thead>
                                <tr>
                                    <th style="width: 300px;">ORDEN DE PREFERENCIA</th>
                                    <th style="width: 400px">NOMBRE DEL PLANTEL EDUCATIVO</th>
                                    <th style="width: 400px">NOMBRE DE LA ESPECIALIDAD</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($aspirante[0]->opcionesEducativas as $opcion)
                                    <tr>
                                        <td>{{ $opcion->preferencia }}</td>
                                        <td>{{ $opcion->seleccionOferta->plantel->descripcion }}</td>
                                        <td>{{ $opcion->seleccionOferta->especialidad->referencia }}</td>
                                    </tr>
                                @endforeach
                                @for ($i = 0; $i < 10 - $aspirante[0]->opcionesEducativas->count(); $i++)
                                    <tr>
                                        <td>&nbsp;</td>
                                        <td>&nbsp;</td>
                                        <td>&nbsp;</td>
                                    </tr>
                                @endfor
                            </tbody>

                        </table>


                        <div>

                        </div>
                        <div class="text-justify" style="font-size: 12px;padding: 10px;">
                            <p style="font-size: 9px; line-height:1">
                                <i>
                                    <strong>Nota: </strong>Los espacios se asignarán tomando en cuenta los aciertos obtenidos en el examen de admisión, y al orden seleccionado de
                                    acuerdo a la preferencia de plantel, habiéndose verificado la capacidad de espacios de cada uno de los planteles públicos.
                                    <i style="color:darkgrey;">En caso de empate se tomará en cuenta los dictámenes de las áreas del examen diagnóstico.</i>
                                </i>
                            </p>


                            <p style="margin-top: 10px">A continuación, aparecen los nombre de la localidad y municipio donde se encuentra la escuela del nivel medio de tu primera
                                preferencia, según escogiste tus opciones educativas.</p>
                        </div>

                        <div class="row" style=" width: 100%; ">
                            <table>
                                <tr style="text-align: center; font-size:12px; font-weight: bold">
                                    <td style="width: 400px;">
                                        {{ $aspirante[0]->opcionesEducativas()->where('preferencia', 1)->first()->seleccionOferta->plantel->localidad->NOM_LOC }}
                                    </td>
                                    <td style="width: 400px;">
                                        {{ $aspirante[0]->opcionesEducativas()->where('preferencia', 1)->first()->seleccionOferta->plantel->localidad->NOM_MUN }}
                                    </td>
                                </tr>
                                <tr style="text-align: center; font-size:11px ">
                                    <td>
                                        LOCALIDAD DONDE SE ENCUENTRA EL PLANTEL DE <br>EDUCACION MEDIA DE SU PRIMERA PREFERENCIA.
                                    </td>
                                    <td>
                                        MUNICIPIO DONDE SE ENCUENTRA EL PLANTEL DE<br>
                                        EDUCACION MEDIA DE SU PRIMERA PREFERENCIA
                                    </td>
                                </tr>
                            </table>
                        </div>

                        <div class="text-justify" style="font-size: 12px;padding: 10px;">
                            Al suscribirse esta solicitud se hace constar que los datos anotados son verdaderos y que se aceptan las condiciones y procedimientos que aparecen en la
                            Convocatoria. La asignación del turno será atribución de las instituciones educativas correspondientes.
                        </div>

                        <div style="margin-top: 10px;  border-bottom-color: lightgrey; border-bottom-style: dashed; border-bottom-width: 1px">
                            <table style="margin-bottom: 10px;">
                                <tr>
                                    <td width="266px">
                                        <hr style="width: 80%">
                                    </td>
                                    <td width="266px">
                                        <hr style="width: 80%">
                                    </td>
                                    <td width="266px">
                                        <hr style="width: 80%">
                                    </td>
                                </tr>
                                <tr style="font-size: 11px; text-align: center; ">
                                    <td>
                                        Firma del alumno(a)
                                    </td>
                                    <td>
                                        @if($aspirante[0]->informacionProcedencia->fecha_egreso == 2019)
                                            Firma y sello de la secundaria de procedencia
                                        @else
                                            Firma del Representante de los Servicios Educativos en el municipio @if($aspirante[0]->informacionProcedencia->fecha_egreso < 2018) o copia
                                            de certificado de estudio. @endif
                                        @endif
                                    </td>
                                    <td>
                                        Firma del padre o tutor
                                    </td>
                                </tr>
                            </table>
                        </div>

                        <div>
                            <table style="font-size: 11px; ">
                                <tr style="font-weight: bold">
                                    <td rowspan="2" width="60px">Pase al examen</td>
                                    <td width="90px">Folio Ceneval</td>
                                    <td width="100px">Fecha y hora</td>
                                    <td width="200px">Sede</td>
                                    <td width="180px">Aula</td>
                                    <td width="70px">No. de Lista</td>
                                </tr>
                                <tr>
                                    <td>{{ $aspirante[0]->folio }}</td>
                                    <td>{{strtoupper($configuracion[0]->valor.', '.$configuracion[1]->valor)}}</td>
                                    <td>{{ $aspirante[0]->paseExamen->aula->edificio->descripcion }}</td>
                                    <td>{{ $aspirante[0]->paseExamen->aula->referencia }}</td>
                                    <td>{{ $aspirante[0]->paseExamen->numero_lista }}</td>
                                </tr>
                            </table>
                        </div>

                        <div class="text-justify">
                            <p style="font-size: 9px; line-height:1" class="mt-2">
                                <i><strong>Nota:</strong>
                                    Después del pago efectuado, de 1 a 3 días, ingresa al portal de registro, para poder imprimir "Pase al Examen". Recuerda que después de la fecha
                                    límite de la referencia bancaria, no podrás realizar el pago. Presentarse 30 minutos, antes del inicio de examen, llevar lápiz del número 2 1/2,
                                    goma y sacapuntas.
                                </i>
                            </p>
                        </div>

                    </div>
                </div>
            </div>
        </div>
    </body>
</html>
